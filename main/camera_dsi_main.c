...
// Anzahl der Buffer für Ping-Pong
#define NUM_FRAME_BUFFERS 2

// Kontext-Struktur, um Buffer-Infos an die Callbacks zu übergeben
typedef struct {
    void *frame_buffers[NUM_FRAME_BUFFERS];
    size_t buffer_size;
    int current_buffer_idx; // Welcher Buffer wird gerade gefüllt?
} camera_context_t;

static camera_context_t cam_ctx = {0};

// Queue für die Kommunikation ISR -> Processing Task
static QueueHandle_t xFrameQueue = NULL;

// Struktur für Frame-Events
typedef struct {
    void *buffer;
    size_t len;
    int buffer_idx;
...
// 1. Der Callback (ISR Kontext) - Meldet fertigen Frame
static bool s_camera_get_finished_trans(esp_cam_ctlr_handle_t handle, esp_cam_ctlr_trans_t *trans, void *user_data)
{
    ...
    // Wir könnten hier den Index aus user_data ableiten, aber der Pointer reicht meistens

    // Sende Event an den Processing Task
    xQueueSendFromISR(xFrameQueue, &evt, &xHigherPriorityTaskWoken);

    return xHigherPriorityTaskWoken == pdTRUE;
}

// 2. Der Callback für neue Buffer (Ping-Pong Logik)
static bool s_camera_get_new_vb(esp_cam_ctlr_handle_t handle, esp_cam_ctlr_trans_t *trans, void *user_data)
{
    camera_context_t *ctx = (camera_context_t *)user_data;

    // Wechsel zum nächsten Buffer (0 -> 1 -> 0 ...)
    ctx->current_buffer_idx = (ctx->current_buffer_idx + 1) % NUM_FRAME_BUFFERS;

    // Dem Treiber den neuen Buffer zuweisen
...
// 3. Der Processing Task
void frame_processing_task(void *arg) {
    frame_event_t evt;
    while (1) {
        if (xQueueReceive(xFrameQueue, &evt, portMAX_DELAY) == pdTRUE) {
            
            // Hier können Sie sicher sein, dass der Treiber gerade in den ANDEREN Buffer schreibt.
            // Dieser Buffer (evt.buffer) gehört jetzt Ihnen zur Verarbeitung.
            
            ESP_LOGI(TAG, "Processing Frame at %p (Size: %u)", evt.buffer, evt.len);
            
            // Beispiel: Verarbeitung simulieren
...
void app_main(void)
{
    ...
    // 1. Queue und Task erstellen
    xFrameQueue = xQueueCreate(5, sizeof(frame_event_t));
    xTaskCreatePinnedToCore(frame_processing_task, "frame_proc", 4096, NULL, 5, NULL, 1);

    // 2. Den ERSTEN Buffer einreihen ("Anfüttern")
    // Wir starten mit Buffer 0
